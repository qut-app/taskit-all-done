...
const Onboarding = () => {
  ...
  const getSteps = (): OnboardingStep[] => {
    if (accountType === 'company') {
      return selectedRole === 'provider'
        ? ['account-type', 'company', 'location', 'verification', 'provider-setup', 'complete']
        : ['account-type', 'company', 'location', 'verification', 'complete'];
    }
    return selectedRole === 'provider'
      ? ['account-type', 'personal', 'location', 'verification', 'provider-setup', 'complete']
      : ['account-type', 'personal', 'location', 'verification', 'complete'];
  };
                ...
                <div className="grid grid-cols-2 gap-3">
                  <motion.div whileTap={{ scale: 0.98 }}>
                    ...
                    >
                      <motion.div 
                        ...
                        animate={{ scale: accountType === 'individual' ? 1.05 : 1 }}
                      >
                        <User className="w-6 h-6" />
                      </motion.div>
                      <h3 className="font-semibold text-foreground">Individual</h3>
                      <p className="text-xs text-muted-foreground mt-1">Personal account for freelancers</p>
                    </Card>
                  </motion.div>
                  ...
                  <motion.div whileTap={{ scale: 0.98 }}>
                    ...
                    >
                      <motion.div 
                        className={`w-12 h-12 rounded-xl flex items-center justify-center mb-3 ${
                          accountType === 'company' ? 'bg-primary text-primary-foreground' : 'bg-muted'
                        }`}
                        animate={{ scale: accountType === 'company' ? 1.05 : 1 }}
                      >
                        <Building2 className="w-6 h-6" />
                      </motion.div>
                      <h3 className="font-semibold text-foreground">Company</h3>
                      <p className="text-xs text-muted-foreground mt-1">Business account with CAC</p>
                    </Card>
                  </motion.div>
                </div>
              </div>
                ...
                <motion.div whileHover={{ scale: 1.01 }}>
                  <Card className="p-4">
                    <div className="flex items-center gap-4">
                      <motion.div 
                        className={`w-20 h-20 rounded-2xl flex items-center justify-center border-2 border-dashed overflow-hidden ${
                          formData.facePhotoUrl ? 'border-success bg-success/10' : 'border-border bg-muted'
                        }`}
                        animate={{ borderColor: formData.facePhotoUrl ? 'hsl(var(--success))' : undefined }}
                      >
                      ...
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          capture="user"
                          className="hidden"
                          onChange={handleUploadFacePhoto}
                        />
                        <Button 
                          variant="outline" 
                          size="sm" 
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          className="hidden"
                          onChange={handleUploadCACDoc}
                        />
                        <Button 
                          variant="outline" 
                          size="sm" 
                          className="mt-2"
                          onClick={() => cacDocRef.current?.click()}
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['online', 'offline', 'both'].map((mode) => {
                    const isSelected = formData.serviceMode === mode;
                    return (
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {DELIVERY_TIMES.map((time) => {
                    const isSelected = formData.deliveryTime === time;
                    return (
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* Footer */}
      <motion.div 
        className="p-6 safe-bottom bg-background border-t border-border/50"
